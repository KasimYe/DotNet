...
namespace Kasim.Core.Common
{
    ...
    {
        internal static readonly HashSet<string> AllowExt = new HashSet<string>() { ".jpe", ".jpeg", ".jpg", ".png", ".tif", ".tiff", ".bmp" }; // ".jpe|.jpeg|.jpg|.png|.tif|.tiff|.bmp";
        public ImageClass()
        { }
        ...
        /// 生成缩略图并返回图片
        /// </summary>
        /// <param name="originalImage">源图Image</param>
        /// <param name="width">缩略图宽度</param>
        /// <param name="height">缩略图高度</param>
        /// <param name="mode">生成缩略图的方式:HW 指定高宽缩放（可能变形）;W 指定宽，高按比例;H 指定高，宽按比例;Cut 指定高宽裁减（不变形）</param>
        /// <returns>缩略图Image</returns>
        public static Image MakeThumbnail(Image originalImage, int width, int height, string mode)
        {
            ...
            {
                //以jpg格式保存缩略图
                //bitmap.Save(@"C:\Users\lip86\Desktop\test\123.jpg", System.Drawing.Imaging.ImageFormat.Jpeg);
            }
            catch (System.Exception e)
            {
                throw e;
